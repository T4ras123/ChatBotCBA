...
async def ask_gpt4o(message: Message):
    ...
    try:
        prompt = [
            ...
            {"role": "system", "content": "You are an assistant who answers users' questions based on the database. The data contains titles, descriptions, and video links."},
            {"role": "user", "content": f"Here are the details about the video: {videos_content}"},
            {"role": "user", "content": f"The user is asking: {user_query}. Using the video data and other database information, answer their query."},
            {"role": "user", "content": "Please respond in plain text and display links as regular text without Markdown formatting."}
        ]
